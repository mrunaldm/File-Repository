...
namespace NoSqlDb
{
  ...
  */
  inline void showHeader(bool showKey = true, std::ostream& out = std::cout)
  {
	  ...
	  out << std::setw(38) << std::left << "Payload";
	  out << "\n  ";
	  if (showKey)
  ...
  {
	  std::string cStatus;
	  CheckinStatus status = el.payLoad().checkInStatus();
	  switch (status)
	  {
	  case open: cStatus = "Open";
		  break;
	  case closed: cStatus = "Closed";
		  break;
	  case pending: cStatus = "Pending";
		  break;
	  default:
		  break;
	  }
	  ...
	  out << std::setw(15) << std::left << el.name().substr(0, 13);
	  out << std::setw(26) << std::left << std::string(el.dateTime());
	  out << std::setw(15) << std::left << el.descrip().substr(0, 15);
	  //out << std::setw(20) << std::left << std::string(el.payLoad()).substr(0, 20);
	  out << std::setw(38) << std::left << std::string(el.payLoad().value());
	  out << "\n";
	  out << "CheckInStatus : " << cStatus;
	  out << "\nPayload Categories : ";
	  std::vector<std::string> categs = el.payLoad().categories();
	  for (auto iter : categs)
	  {
		  out << iter << "\t";
	  }
	  Children children = el.children();

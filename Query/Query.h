...
namespace NoSqlDb
{
  ...
  public:
	...
	Query& keyBasedMatch(std::string&);
  ...
  */
  ...
  //Query based on regex patter on keys
  template<typename T>
  Query<T>& Query<T>::keyBasedMatch(std::string& pattern)
  {
	  for (auto iterator : db_)
	  {
		  if (std::regex_match(iterator.first, pattern))
